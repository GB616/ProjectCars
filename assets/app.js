...
function addVote(slug, vote, ups, downs)
{
     if(vote == 'up')
        ups+=1;
    else if(vote == 'down')
    ...
    const request = new Request('/project/' + slug +'/' +vote,  { method: 'POST' });   
    fetch(request)
    .then(reponse => {
    console.log('We vote!');
   
    var div = document.getElementById('votesRatio');
    div.innerHTML = `<a class="text-wrap" onclick="addVote('{{car.slug}}', 'up', {{car.voteUp}}, {{car.voteDown}} )"> 
...
function notlogged()
{
    alert('you need to be logged to use this function')
}